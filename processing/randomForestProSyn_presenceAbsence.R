...
#these are the Pro or Syn sequences
seqG %>% filter(is.na(V1))

#excludes the Pro or Syn sequences
...
#correct
seqG %>% filter(culture == "Pro") %>% distinct(sample)
...
write_csv(seqG, "seqGForPresenceAbsenceRandomForestPlot.csv")

head(seqG)

#makes presence, absence variable to replace relative abundance variable
seqG <- seqG %>% mutate(presence = 1)

seqG <- seqG %>% select(-propSample)

head(seqG)

#spreads seqG so that the sequences that are not in a sample will have 0 abundance for that sample
seqSpread <- seqG %>% spread(key = featureContig, value = presence, fill = 0)

dim(seqSpread)

#gathers seqG again, now with rows for sequences that are not present in a sample
seqG <- seqSpread %>% gather(3:237, key = "V1", value = "presence")
...
#loads relative abundance data from above
seqGOld <- read_csv("seqGForPresenceAbsenceRandomForestPlot.csv")

head(seqGOld)

#spreads seqGOld so that sequences not present in a sample have 0 relative abundance
seqSpreadOld <- seqGOld %>% spread(key = featureContig, value = propSample, fill = 0)

dim(seqSpreadOld)

#gathers seqGOld, now with rows for sequences that are not present in a sample
seqGOld <- seqSpreadOld %>% gather(3:237, key = "V1", value = "propSample")
...
toPlot <- seqGOld %>% inner_join(impDF[1:20,], by = c("V1"))
...
#15/20 of the most important variables based on presence absence are also in the 20 most important 
#variables based on relative abundance
...
ggsave("randomForestBasedOnPresenceAbsence_20MostImportantVariablesToDetermineProOrSynCulture.png", dpi = 600, height = 10, width = 14)
ggsave("randomForestBasedOnPresenceAbsence_20MostImportantVariablesToDetermineProOrSynCulture.svg", dpi = 600, height = 10, width = 14)
