...
#these are the classes and their clusters that are found in both the sean and diatom samples
...
#102 clusters were just found in sean samples
biomG %>% semi_join(foundInOneSource_noProSyn, by = c("OTU_ID")) %>% filter(sampleSource == "sean") %>% distinct(OTU_ID) %>% summarize(n = n())

#17 clusters were just found in diatom samples
biomG %>% semi_join(foundInOneSource_noProSyn, by = c("OTU_ID")) %>% filter(sampleSource == "diatom") %>% distinct(OTU_ID) %>% summarize(n = n())
...
biomG %>% distinct(class) %>% arrange(class) %>% select(class) %>% as.list()

order <-  c("A712011", "Alphaproteobacteria", "Betaproteobacteria", "Cytophagia", 
            "Deltaproteobacteria", "Flavobacteriia", "Gammaproteobacteria", "[Leptospirae]",                 
...
ggsave("diatomAndSeanTaxonomyStackedBarPlot_withProSyn.svg", dpi = 600, height = 6, width = 15)
...
noProSyn %>% distinct(class) %>% arrange(class) %>% select(class) %>% as.list()
...
ggsave("diatomAndSeanTaxonomyStackedBarPlot_noProSyn.svg", dpi = 600, height = 6, width = 15)
