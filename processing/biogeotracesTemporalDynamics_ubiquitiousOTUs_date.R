...
colnames(toPlot)
toPlot %>% group_by(sample, OTU_ID, biogeotracesSequence, biogeotracesSample) %>% 
...
#n is the abundance of the OTU in the given biogeotraces sample
#these are for the rows for biogeotraces samples that didn't have
#the ubiquitious OTUs
head(ubiSummary)
ubiSummary %>% distinct(OTU_ID, n)
ubiSummary %>% distinct(OTU_ID, biogeotracesSample, n)
ubiSummary %>% filter(is.na(n))
ubiSummary %>% filter(is.na(n)) %>% select(n, numSeq)
...
ubiSummary %>% filter(propOfBiogeotraces == 0)
...
#gets just the rows in ubiSummary for biogeotraces samples that are in HOT or BATS
ubiSummary <- ubiSummary %>% filter(geo_loc_name %in% 
c("Atlantic Ocean: Sargasso Sea\\, BATS", "Pacific Ocean: North Pacific Subtropical Gyre\\, Station ALOHA"))

#some of the ubiquitious OTUs were lost because they were only in non-HOT, non-BATS
...
#adds the ubiquitious OTUs that were left out back to ubiSummary
nrow(ubiSummary)
...
#gets rid of unnecessary variables so that I can spread data correctly
ubiSummary <- ubiSummary %>% select(OTU_ID:biogeotracesSample)
head(ubiSummary)

#there is only one row for each OTU_ID, biogeotracesSample pair
ubiSummary %>% group_by(OTU_ID, biogeotracesSample) %>% summarize(n = n()) %>% filter(n != 1)

#spreads data so that there is a propOfBiogeotraces value of 0 when the OTU is not 
...
#now each OTU has a propOfBiogeotraces value for each of the HOT 
#and BATS biogeotraces samples
ubiSummary %>% group_by(OTU_ID) %>% summarize(n = n())
...
#adds geolocation, collection date of biogeotraces samples back to meta
nrow(ubiSummary)
ubiSummary <- ubiSummary %>% left_join(meta, by = c("biogeotracesSample" = "Run"))
nrow(ubiSummary)
...
#makes a collection month, day variable
ubiSummary <- ubiSummary %>% mutate(monthDay = str_replace_all(collection_date, "[0-9]{4}-", ""))

ubiSummary %>% select(collection_date, monthDay) %>% head()
ubiSummary %>% distinct(collection_date, monthDay) %>% head()

ubiSummary %>% filter(is.na(monthDay))

#ubiSummary %>% distinct(monthDay) %>% arrange(monthDay) %>% View()
...
##puts the OTUs in order of how many sean culture samples they were found in
ubi
...
#makes a collection month, day, year variable
ubiSummary <- ubiSummary %>% mutate(monthDayYear = str_c(monthDay, str_extract(collection_date, "[0-9]*"), sep = "-"))

#makes a collection year variable
ubiSummary <- ubiSummary %>% mutate(year = str_extract(collection_date, "[0-9]*"))

ubiSummary %>% select(collection_date, monthDay, monthDayYear, year) %>% head()
ubiSummary %>% distinct(collection_date, monthDay, monthDayYear, year) %>% head()
#ubiSummary %>% distinct(collection_date, monthDay, monthDayYear, year) %>% View()

head(ubiSummary)

colnames(ubiSummary)

ubiSummary %>% filter(is.na(collection_date))
ubiSummary %>% filter(is.na(monthDay))
ubiSummary %>% filter(is.na(monthDayYear))
ubiSummary %>% filter(is.na(year))

ubiSummary %>% distinct(year) %>% arrange(year)

##puts the collection dates in order, separately by year

order2003 <- ubiSummary %>% arrange(monthDay) %>% filter(year == "2003") %>% select(monthDayYear) %>%
  distinct(monthDayYear)
order2003

order2004 <- ubiSummary %>% arrange(monthDay) %>% filter(year == "2004") %>% select(monthDayYear) %>%
  distinct(monthDayYear)
order2004

order2009 <- ubiSummary %>% arrange(monthDay) %>% filter(year == "2009") %>% select(monthDayYear) %>%
  distinct(monthDayYear)
order2009

#puts all of the collection dates in order
order <- bind_rows(order2003, order2004, order2009)

#makes the ordered collection dates into a list
order
order <- order %>% select(monthDayYear)
order
order <- order %>% as.list()
order
order <- order$monthDayYear
order
length(order)

ubiSummary %>% distinct(monthDayYear)
ubiSummary %>% distinct(monthDayYear) %>% nrow()

#puts the collectiond ates in ubiSummary in order
ubiSummary$monthDayYear <- factor(ubiSummary$monthDayYear, levels = order)

levels(ubiSummary$monthDayYear)
levels(ubiSummary$monthDayYear) %>% length()

#plot BATS
ubiSummary %>% 
  filter(geo_loc_name == "Atlantic Ocean: Sargasso Sea\\, BATS") %>%
  ggplot() + geom_line(aes(x = monthDayYear, y = propOfBiogeotraces, group = otuSequence)) +
  geom_point(aes(x = monthDayYear, y = propOfBiogeotraces), size = 1) +
  ...
  labs(title = "BATs: OTUs present in the most cultures,\narranged by number of cultures they were found in") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + labs(x = "date")

ggsave("seanSeqsNoProSyn_biogeotracesSamplesWithUbiquitousOTUs_byDateBATS.png", dpi = 600, height = 10, width = 20)
ggsave("seanSeqsNoProSyn_biogeotracesSamplesWithUbiquitousOTUs_byDateBATS.svg", dpi = 600, height = 10, width = 20)
...
ubiSummary %>% 
  ...
  ggplot() + geom_line(aes(x = monthDayYear, y = propOfBiogeotraces, group = otuSequence)) +
  geom_point(aes(x = monthDayYear, y = propOfBiogeotraces)) +
  labs(y = expression(paste("Proportion of non-Pro., non-Syn. sequences in BioGeoTraces sample"))) + 
  theme(legend.key.size = unit(5, "mm")) + facet_wrap(~otuSequence, scales = 'free') +
  labs(title = "HOT: OTUs present in the most cultures,\narranged by number of cultures they were found in") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + labs(x = "date")

ggsave("seanSeqsNoProSyn_biogeotracesSamplesWithUbiquitousOTUs_byDateHOT.png", dpi = 600, height = 10, width = 20)
ggsave("seanSeqsNoProSyn_biogeotracesSamplesWithUbiquitousOTUs_byDateHOT.svg", dpi = 600, height = 10, width = 20)
